...
async function createSanityCalendar (graphql, actions) {
  const {createPage} = actions
  const result = await graphql(`
    {
      allSanityCalendar(filter: { slug: { current: { ne: null } } }) {
        edges {
          node {
            id
            slug {
  ...
  const calendarEdges = (result.data.allSanityCalendar || {}).edges || []
...
exports.createPages = async ({graphql, actions}) => {
  ...
  await createSanityCalendar(graphql, actions)
}
